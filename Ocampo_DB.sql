...
CREATE PROCEDURE sp_AGREGAR_PRODUCTO(
	@MARCA VARCHAR(20), @CATEGORIA VARCHAR(20), @DESCRIPCION VARCHAR(60), @STOCK_ACTUAL INT,
	@STOCK_MINIMO INT, @PRECIO FLOAT
)AS
BEGIN
DECLARE @IDM INT = (SELECT ID FROM MARCAS WHERE NOMBRE=@MARCA)
DECLARE @IDC INT = (SELECT ID FROM CATEGORIAS WHERE NOMBRE=@CATEGORIA)
	BEGIN TRY
	INSERT INTO PRODUCTOS(DESCRIPCION,STOCK_ACTUAL,STOCK_MINIMO,PRECIO_COMPRA,IDCATEGORIA,IDMARCA)
	VALUES (@DESCRIPCION,@STOCK_ACTUAL,@STOCK_MINIMO,@PRECIO,@IDC,@IDM)
	END TRY
...
DECLARE @IDC INT = (SELECT ID FROM CATEGORIAS WHERE NOMBRE=@CATEGORIA)
	...
	PRECIO_COMPRA=@PRECIO,IDCATEGORIA=@IDC,IDMARCA=@IDM WHERE ID=@ID
	END
	END TRY
